...
BEGIN
  ...
  END IF;
  IF new.passport NOT REGEXP '[A-Z]{2}-[0-9]{6}'
	THEN SIGNAL SQLSTATE "45000"
	SET MESSAGE_TEXT = "invalid passport";
  END IF;
  if (new.first_name not in ('Vasyl', 'Anna', 'Iryna', 'Andriy', 'Uriy')) then
        signal sqlstate '45000'
...
DROP TRIGGER IF EXISTS logUpadatePositionTrigger;
DELIMITER //
CREATE TRIGGER logUpadatePositionTrigger
BEFORE INSERT
ON position FOR EACH ROW
BEGIN
    INSERT INTO log_upadate_position (changes) VALUES (now());
END //
DELIMITER ;
